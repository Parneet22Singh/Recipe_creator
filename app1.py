...
class ExaTools:
    ...
    def search_recipes(self, query: str) -> str:
        ...
        for r in results:
            ...
            if author:
                markdown += f" by *{author}*"
            if published:
                markdown += f" ({published})"
            markdown += "\n"
...
with st.form("recipe_form"):
    user_query = st.text_area("🥕 What ingredients and preferences do you have?", placeholder="e.g. I have mushrooms, garlic, pasta. Want a 20-minute vegetarian meal.")
    ...
    if not user_query.strip():
        ...
        if show_real:
            with st.spinner("🔍 Searching Exa for matching real-world recipes..."):
                exa_result = exa_tools.search_recipes(user_query)
                st.markdown(exa_result)
